...
// No collector by default, deliberately: nothing leaves the browser unless
// this deployment is configured to send it somewhere.
...
const INSTALL_ID_KEY = "motionscript.installId";
const ENABLED_KEY = "motionscript.telemetry";
...
function mirrorToDesktop(enabled: boolean) {
  try {
    window.motionscriptDesktop?.setTelemetryEnabled(enabled);
  } catch {
