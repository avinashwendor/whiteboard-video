...
export const UI_LOCALE_STORAGE_KEY = "motionscript.ui-locale";
...
export function buildLocaleBootScript(): string {
  ...
  // Falls back to the pre-rename key: this runs in the document head, before
  // any module — and so before `migrateLegacyPreferences` has had a chance to
  // move it. Without the fallback, the first load after the rename would reset
  // a chosen interface language to the browser's.
  return `(function(){try{var p=localStorage.getItem(${JSON.stringify(UI_LOCALE_STORAGE_KEY)})||localStorage.getItem("rescript.ui-locale")||"system";var l=p;if(p==="system"){var a=navigator.languages&&navigator.languages.length?navigator.languages:[navigator.language];l=${JSON.stringify(DEFAULT_UI_LOCALE)};for(var i=0;i<a.length;i++){var v=(a[i]||"").toLowerCase().replace(/_/g,"-");${rules}}}else{var allowed=${JSON.stringify([...UI_LOCALES])};if(allowed.indexOf(l)<0)l=${JSON.stringify(DEFAULT_UI_LOCALE)}}document.documentElement.lang=l}catch(e){}})();`;
}
