...
import { paintFrame } from "@/motionscript/lib/overlay/frame";
import { loadImage, nudgeClips } from "@/motionscript/lib/overlay/render";
import { ensureTypefaces } from "@/motionscript/lib/overlay/fonts";
import { transitionAt } from "@/motionscript/lib/overlay/timeline";
import { useOutputTimeline } from "@/motionscript/hooks/useOverlayTimeline";
import { useAudioMix } from "@/motionscript/hooks/useAudioMix";
import { useFreezeFrames } from "@/motionscript/hooks/useFreezeFrames";
import type { OverlayElement, Rect } from "@/motionscript/lib/overlay/types";
...
export default function OverlayStage({
  ...
  const acceptDrop = useCallback(
    async (event: React.DragEvent) => {
      ...
      const url = event.dataTransfer.getData("application/x-motionscript-image");
      if (url) {
        const w = 0.34;
        store.addImage(url, {
          start: at,
