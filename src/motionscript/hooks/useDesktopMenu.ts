...
import { deleteProject, listProjects } from "@/motionscript/lib/projects";
import { useEditorStore } from "@/motionscript/lib/store";
import type { MenuCommand } from "@/motionscript/types/motionscript-desktop";
import { useI18n } from "@/motionscript/components/I18nProvider";
...
export function useDesktopMenu(openFilePicker: () => void, ready: boolean): void {
  ...
  const syncRecents = useCallback(async () => {
    if (!window.motionscriptDesktop) return;
    try {
      const projects = await listProjects();
      window.motionscriptDesktop.setRecentProjects(
  ...
  const closeProject = useCallback(async () => {
    ...
    try {
      const { flushProjectAutosave } = await import("@/motionscript/lib/autosave");
  ...
  useEffect(() => {
    if (!isElectron) return;
    window.motionscriptOpenFilePicker = openFilePicker;
    return () => {
      delete window.motionscriptOpenFilePicker;
    };
  }, [openFilePicker]);
  ...
  useEffect(() => {
    const desktop = window.motionscriptDesktop;
    if (!desktop) return;
