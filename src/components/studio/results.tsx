 ...
 * A board of frames.
 *
 * Every frame is shown from the moment it is planned, holding a grain plate
 * until its picture lands. Hiding the unfinished ones would make a six-frame
 * board look like a one-frame board that keeps growing.
 */
export function StoryboardResult({ generation }: { generation: Generation }) {
  const board = generation.project;
  ...
  if (!board) {
    return (
      <div className="p-4 sm:p-5">
        ...
        <p className="pt-3 text-[12px] text-faint">{generation.stage ?? "Breaking it into frames"}</p>
      </div>
    );
  }

  const drawn = board.scenes.filter((frame) => frame.image).length;

  return (
    <>
      <StatBar>
        <Stat label="Frames" value={String(board.scenes.length)} />
        <Stat label="Drawn" value={`${drawn}/${board.scenes.length}`} />
        {generation.meta.model ? <Stat label="Planner" value={generation.meta.model} /> : null}
      </StatBar>

      {board.description ? (
        <p className="border-b border-line px-4 py-3 text-[13.5px] leading-relaxed text-muted sm:px-5">
          {board.description}
        </p>
      ) : null}

      <div className="grid gap-px bg-[var(--border)] sm:grid-cols-2">
        {board.scenes.map((frame, index) => (
          <figure key={index} className="flex flex-col bg-surface p-4">
            <div className="relative aspect-video w-full bg-[#08080a]">
              {frame.image ? (
                // eslint-disable-next-line @next/next/no-img-element -- object URLs bypass the optimiser
                <img
                  src={frame.image.url}
                  alt={frame.heading}
                  className="animate-fade absolute inset-0 size-full object-cover"
                />
              ) : frame.status === "error" ? (
                <span className="absolute inset-0 grid place-items-center px-4 text-center text-[11.5px] leading-relaxed text-danger">
                  {frame.error ?? "This frame didn't draw."}
                </span>
              ) : (
                <GrainShimmer
                  className="absolute inset-0"
                  sweep={frame.status === "running" ? 1.9 : 3.4}
                  intensity={frame.status === "running" ? 0.55 : 0.3}
                />
              )}

              <span className="absolute left-2 top-2 bg-[#0a0a0b]/80 px-1.5 py-0.5 font-mono text-[9.5px] tracking-[0.14em] text-[#c9c9c4]">
                {String(index + 1).padStart(2, "0")}
              </span>
            </div>

            <figcaption className="pt-3">
              <p className="text-[13.5px] font-medium leading-snug text-ink">{frame.heading}</p>
              <p className="mt-1 line-clamp-3 text-[12px] leading-relaxed text-faint">
                {frame.narration}
              </p>
            </figcaption>
          </figure>
