...
import {
  ...
  siftOps,
  type AgentOp,
} from "@/motionscript/lib/overlay/ops-schema";
import { verifyPlan, type PlanWorld } from "@/motionscript/lib/overlay/verify";
import { checkCraft } from "@/motionscript/lib/overlay/craft";
import { describeTemplates } from "@/motionscript/lib/overlay/templates";
import { describeSubtitlePresets } from "@/motionscript/lib/overlay/subtitles";
import { describeSfx } from "@/motionscript/lib/overlay/sfx";
import { describeAmbients } from "@/motionscript/lib/overlay/ambient";
import { describeTypefaces } from "@/motionscript/lib/overlay/typefaces";
import {
  describeFrame,
  describeVision,
  readNearest,
  type WireFrameRead,
} from "@/motionscript/lib/overlay/vision";
...
{"op":"subtitles","action":"style","preset":"punch","typeface":"anton","color":"#ffffff","highlight":"#ffd60a","size":"l","position":"center","uppercase":true,"wordsPerCue":2,"emphasis":"auto","emphasisColor":"#4ade80","keywords":["motionscript","free"],"activeScale":1.12}
...
export interface MotionScriptAgentContext {
...
export interface MotionScriptExchange {
  instruction: string;
...
export interface MotionScriptExemplar {
  instruction: string;
  title: string;
  detail: string;
...
function learnedBlock(
  exemplars: MotionScriptExemplar[] | undefined,
...
export interface MotionScriptAgentInput {
  instruction: string;
  context: MotionScriptAgentContext;
  /**
   ...
   */
  exemplars?: MotionScriptExemplar[];
...
export interface MotionScriptStep {
  title: string;
  detail: string;
  ops: AgentOp[];
}
...
export interface MotionScriptTraceEntry {
  tool: string;
  detail: string;
}

export interface MotionScriptPlan {
  summary: string;
  findings: string[];
  steps: MotionScriptStep[];
  ops: AgentOp[];
  rejected: string[];
  /** What it looked at, for the log. */
  trace: MotionScriptTraceEntry[];
...
function describeAnalysis(
  analysis: NonNullable<MotionScriptAgentContext["analysis"]>
): string {
  return [
...
function describeProject(context: MotionScriptAgentContext): string {
...
export async function planMotionScriptEdit(
  input: MotionScriptAgentInput
): Promise<MotionScriptPlan> {
...
still what the video is *about*.`,
  ...
  const trace: MotionScriptTraceEntry[] = [];
  /**
  ...
  for (let turn = 0; turn < MAX_TURNS; turn += 1) {
    ...
    // Set MOTIONSCRIPT_AGENT_DEBUG to a path to see what the model actually said.
    ...
    if (process.env.MOTIONSCRIPT_AGENT_DEBUG) {
      const { appendFileSync } = await import("node:fs");
      appendFileSync(
        process.env.MOTIONSCRIPT_AGENT_DEBUG,
        `\n=== turn ${turn} ===\n${result.text}\n`
      );
    }
  ...
  throw new AppError("malformed_response", {
    ...
    detail: `motionscript plan gave up after ${MAX_TURNS} turns (${looks} looks, ${wasted} wasted, tools ${toolsClosed ? "closed" : "open"}): ${problem}`,
  });
}
