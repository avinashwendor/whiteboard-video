...
import { deleteProject, listProjects } from "@/rescript/lib/projects";
import { useEditorStore } from "@/rescript/lib/store";
import type { MenuCommand } from "@/rescript/types/rescript-desktop";
import { useI18n } from "@/rescript/components/I18nProvider";
...
export function useDesktopMenu(openFilePicker: () => void, ready: boolean): void {
  ...
  const closeProject = useCallback(async () => {
    ...
    try {
      const { flushProjectAutosave } = await import("@/rescript/lib/autosave");
